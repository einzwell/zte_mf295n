...
var inputContext = "", currentKeyID = 0, rd0 = "", rd1 = "";
require.config({
    paths: {
    ...
    if (zte_web_ui_is_test && (window.simulate = a), "CPE" == i.PRODUCT_TYPE) {
        ...
        e.getOpMode({}, function (e) {
            ...
            n({menu: "config/" + i.DEVICE + "/" + o, config: "config/" + i.DEVICE + "/config"})
        })
    } else n({menu: "config/" + i.DEVICE + "/menu", config: "config/" + i.DEVICE + "/config"})
});
